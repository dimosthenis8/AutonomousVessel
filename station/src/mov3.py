...
rospy.init_node('station3')
def startBoth(speed):
	rate = rospy.Rate(10)
	publisher1 = rospy.Publisher('/boat4/thrusters/right_thrust_cmd', Float32, queue_size=1)
	publisher2 = rospy.Publisher('/boat4/thrusters/left_thrust_cmd', Float32, queue_size=1)
	rate.sleep()
...
def startRightEngine(speed):
	rate = rospy.Rate(10)
	publisher = rospy.Publisher('/boat4/thrusters/right_thrust_cmd', Float32, queue_size=1)
	rate.sleep()
...
def startLeftEngine(speed):
	rate = rospy.Rate(10)
	publisher = rospy.Publisher('/boat4/thrusters/left_thrust_cmd', Float32, queue_size=1)
	rate.sleep()
	publisher.publish(speed)
	rate.sleep()
...
def startLateralEngine(speed):
	rate = rospy.Rate(10)
	publisher = rospy.Publisher('/boat4/thrusters/lateral_thrust_cmd', Float32, queue_size=1)
	rate.sleep()
	publisher.publish(speed)
...
def getorientation():
	...
	rospy.Subscriber('/boat4/sensors/imu/imu/data', Imu, temp)
	rate.sleep()
...
def getLatAndLong():
	...
	rospy.Subscriber('/boat4/sensors/gps/gps/fix', NavSatFix, temp)
	rate.sleep()
	return (temp1,temp2)
